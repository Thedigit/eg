...
function doPickup(){
    ...
    $(".ShipOption[option!=1]").fadeOut(function(){$(".ShipOption[option=1]").fadeIn(equalize);});
    $(".StaticShipping").html("Pickup");

}
function doUPS(){

    $(".ShipOption[option!=2]").fadeOut(function(){$(".ShipOption[option=2]").fadeIn(equalize);});
...
function updatedZip(){
    ...
    if(firstLoad){return;}
     if($("input:radio[name=MainShipOption]").is(":checked")){ syncCart(true);}
}
...
{      try{
    ...
    refreshTotal();
    updateTotal();
}catch(e){document.location.reload(true);}
}
