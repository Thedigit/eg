...
<%
 ...
 int orderCount = Integer.valueOf(Utilities.fetch2("SELECT COUNT(*) as `c` FROM egprint.invoices i WHERE i.Customer='"+activeCustomer.get("insideId")+"'").get(0).get("c"));
      ...
      function loadInfo(id){
         ...
         var params = new Object();
          ...
          $.getJSON("utils/?callback=?",params,function(data){
              ...
              $("#pdSize").html(data.Width + "\""+ "x"+data.Height+ "\"");
      ...
      function updateProgress()
  {
   ...
   $.getJSON('<%=activeCompany.optString("UploadDomain","")%>/UploadStatus?a='+currentProgressTicket+'&jsoncallback=?'
            ...
            if (data.done=="true")
            {
              ...
              progBar.style.backgroundImage="url(https://upload.egdeal.com/upload/progress.png)";
...
<div id="myAccount">
    <div class="BreadCrumb">
        ...
        <span class="BCDivider">&gt;</span>
        <span class="BCLight">Invoices</span>
    ...
    <div id="history" class="MAInfoDiv">
        <div class="Title2">
        <img src="images/MyAccount.png" alt=""> Invoices
        <span class="Pages">
            <%if(currentPage>1){%><a href="invoices.jsp?page=1"><img src="images/endArrow_L.gif" alt=""></a><%}else{%><img src="images/endArrow_L_disabled.gif" alt=""><%}%>
            <%if(currentPage>1){%><a href="invoices.jsp?page=<%=Math.max(1,currentPage-1)%>"><img src="images/singleArrow_L.gif" alt=""></a><%}else{%><img src="images/singleArrow_L_disabled.gif" alt=""><%}%>
            <%for(int i = Math.max(1,currentPage-displayable) ; i <= Math.min(totalPages,currentPage+displayable);i++){%> <a href="invoices.jsp?page=<%=i%>" class="PageNumber <%if(i==currentPage){%>PageSelected<%}%>"><%=i%></a> <%}%>
            <%if(currentPage<totalPages){%><a href="invoices.jsp?page=<%=Math.min(totalPages,currentPage+1)%>"><img src="images/singleArrow_R.gif" alt=""></a><%}else{%><img src="images/singleArrow_R_disabled.gif" alt=""><%}%>
            <%if(currentPage<totalPages){%><a href="invoices.jsp?page=<%=totalPages%>"><a href="invoices.jsp?page=<%=totalPages%>"><img src="images/endArrow_R.gif" alt=""></a><%}else{%><img src="images/endArrow_R_disabled.gif" alt=""><%}%> </span>
        </div>

        <%
            ArrayList<HashMap<String,String>> recentOrders = Utilities.fetch3("SELECT i.*, DATE(i.date) as `invDate` FROM egprint.invoices i WHERE  i.Customer='"+activeCustomer.get("insideId")+"'ORDER BY i.Date DESC LIMIT "+limit+" OFFSET "+offset);
        ...
        <table class="ListTable">
            <thead>
             <tr>
                ...
                <th style="width:120px">Printable Version</th>
                <th></th>
            </tr>
        ...
        <%for(HashMap<String,String> order:recentOrders){
            String selectQuery = "SELECT  ord.*, hh.Description as `HoldType`, hh.FilesRequired, qq.description as `qtyDesc`, qq.TurnAround, qq.TurnAroundUnit, p.ID as `pId`, p.ProductName as `Product` , p.Description,  c.description as `Category`,  st.description as `Stock`, pr.description as `Press`, s.description as `Sides`, w.description as `Width`, h.description as `Height`, l.description as `Lamination`, u.description as  `UV`, score.description as `Score_Diecut`, f.description as `Fold`, o.description as `Other`  " +
                       ...
                       " AND ord.Product=p.id AND ord.quantities = qq.id  AND ord.id=oii.order AND oii.invoice=i.id AND i.id='"+order.get("id")+"'";

             selectQuery = "SELECT  qq.description as `qtyDesc`,  c.description as `Category`  " +
                                "FROM  egprint.ordersininvoice oii, egprint.invoices i, egprint.orders ord, egprint.quantities qq, egprint.product p,  egprint.category c " +
                                "WHERE  p.category=c.id "+
                       " AND ord.Product=p.id AND ord.quantities = qq.id  AND ord.id=oii.order AND oii.invoice=i.id AND i.id='"+order.get("id")+"' GROUP BY `Category`";

             selectQuery = "SELECT  COUNT(*) as `itemCount`  " +
                                "FROM  egprint.ordersininvoice oii, egprint.invoices i, egprint.orders ord, egprint.quantities qq, egprint.product p,  egprint.category c " +
                                "WHERE  p.category=c.id "+
                       " AND ord.Product=p.id AND ord.quantities = qq.id  AND ord.id=oii.order AND oii.invoice=i.id AND i.id='"+order.get("id")+"'";

           ArrayList<HashMap<String,String>> desc = Utilities.fetch3(selectQuery);
           String itemCount = "0";
            if(desc.size()>0){itemCount = desc.get(0).get("itemCount");}
            StringBuffer invDesc =new StringBuffer("");
            boolean first=true;
           for(HashMap<String,String> d: desc){
                  if(!first){invDesc.append("<br> ");}
                  first=false;
                  //invDesc.append(d.get("qtyDesc")+" ");
                  invDesc.append(d.get("itemCount"));
           }
           selectQuery ="SELECT TRUNCATE(SUM(o.Taxable*.07*o.Price+o.Price +o.ShippingPrice),2) as `OrderTotal` FROM egprint.orders o,egprint.ordersininvoice oii, egprint.invoices i WHERE  o.id=oii.order AND oii.invoice=i.id AND i.id='"+order.get("id")+"'";


            HashMap<String,String> tot = Utilities.fetch3(selectQuery).get(0);
            if(tot.get("OrderTotal")==null){tot.put("OrderTotal","0.00");}

            if(itemCount.equals("0")){continue;}

        %>
            <tr order="<%=order.get("id")%>" onclick="document.location='viewInvoice.jsp?j=<%=order.get("id")%>'" >
                <td></td>
                <td>&nbsp;&nbsp;<%=order.get("Number")%></td>
                <td><%=Utilities.s2c(order.get("invDate"))%><!--a class="Reorder" href="">Reorder</a--></td>
                <td><%=itemCount%> items</td>
                <td>$<%=tot.get("OrderTotal")%></td>
                 ...
                 <div><iframe class="UpFrame" id="up_1" name="up_1" count='0' key='1'></iframe></div>
                       <form id="form_1" action="<%=activeCompany.optString("UploadDomain","")%>/FileUpload" method="POST" enctype="multipart/form-data" target="up_1" key="1" ticket="" onsubmit="return false;">
                        ...
                        <table class="FileUpload" id="fu_1">
                            ...
                            <tr><td class="UpTitle">Artwork Front:</td><td style="position:absolute;"><input type="file" class="RealFile" name="file1" key="1f"></td></tr>
                <tr id="upSide2"><td class="UpTitle">Artwork Back: </td><td style="position:absolute;"><input type="file" class="RealFile" name="file2" key="1b" skip="true"></td></tr>
